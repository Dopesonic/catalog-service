...
func (s *srv) Update(ctx context.Context, guid uuid.UUID, req entity.RequestCategoryUpdate) (entity.Category, error) {
	...
	existingCats, err := s.repoCategory.List(ctx, &req.Name)
	if err != nil {
		return entity.Category{}, err
	}
	...
	currentCat.Name = req.Name
	currentCat.UpdatedAt = time.Now()

	if err := s.repoCategory.Update(ctx, currentCat); err != nil {
		return entity.Category{}, err
	}
	return currentCat, nil
}
